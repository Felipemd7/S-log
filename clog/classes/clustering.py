...
class CA(nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(device, (str, bool)) and device in ('cuda', 'CUDA', 'gpu', 'GPU', True) and not self.use_cuda:
        ...
        self.weights_initial_embedding = self.weights_initial_embedding.to(self.device)
        ...
        self.Encoder = make_model(src_vocab=input_log_events,
                                  ...
                                  initial_weights=self.weights_initial_embedding).to(self.device)
    ...
    def _resolve_device(self, device):
        if isinstance(device, torch.device):
            ...
            target = torch.device('cuda' if device and torch.cuda.is_available() else 'cpu')
        else:
            target = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    def _loss(self, X, cluster_id, label):
        ...
        # latent_X = self.Encoder.forward(X.to(self.device), None, None, None)
        # next_sentance_prediction = self.Encoder.generator(latent_X)
        # preds = torch.argmax(next_sentance_prediction, axis=1).view(-1, 1)
        #
        # rec_loss = self.criterion(next_sentance_prediction, label.to(self.device))
        #
        ...
        src_mask = create_mask(X.to(self.device), self.PAD_TOKEN)
        latent_X = self.Encoder(X.to(self.device), src_mask=src_mask)
        masked_token_pred = self.Encoder.generator.forward(latent_X)

        preds = torch.argmax(masked_token_pred, axis=1).view(-1, 1)
        rec_loss = self.criterion(masked_token_pred.contiguous(), label.to(self.device).contiguous())
    ...
    def init_clusters_train(self, train_dataloader, test_dataloader, epoch=100, verbose=True):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(train_dataloader):
                b_input, label = batch
                src_mask = create_mask(b_input.to(self.device), self.PAD_TOKEN)
                latent_X = self.Encoder(b_input.to(self.device), src_mask=src_mask)
                latent_X = latent_X[:, 0, :]
    ...
    def fit_warmup(self, train_dataloader, log_epoch, logging_freq):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            src_mask = create_mask(b_input.to(self.device), self.PAD_TOKEN)
            latent_X = self.Encoder(b_input.to(self.device), src_mask=src_mask)
            rec_loss = self._loss_warmup(latent_X, label.to(self.device))
